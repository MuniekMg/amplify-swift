...
class PredictionsPluginConfigurationTests: XCTestCase {
    ...
    func testConfiguration() {
        ...
        do {
            ...
            XCTAssertEqual(configuration.identify.identifyLabels?.type, .labels, "Label type should match")

            XCTAssertEqual(configuration.convert.translateText?.sourceLanguage, .finnish)
            XCTAssertEqual(configuration.convert.translateText?.targetLanguage, .dutch)
    ...
    /// Given: A configuration consisting of only a `defaultRegion`
    /// When: invoking `AWSPredictionsPlugin().configure(using:)` with that configuration
    /// Then: The plugin should successfully configure and its `defaultRegion` value should match the config
    func test_configureWithDefaultRegion() throws {
        let region = "us-east-1"
        let configuration: JSONValue = ["defaultRegion": .string(region)]
        let plugin = AWSPredictionsPlugin()
        try plugin.configure(using: configuration)
        XCTAssertEqual(plugin.config.defaultRegion, region)
    }
}

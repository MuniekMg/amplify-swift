...
public typealias GraphQLResolver = (_ object: JSONObject, _ info: GraphQLResolveInfo) -> ResultOrPromise<JSONValueAny?>
...
public final class GraphQLExecutor {
  ...
  private func complete<Accumulator: GraphQLResultAccumulator>(value: JSONValueAny, ofType returnType: GraphQLOutputType, info: GraphQLResolveInfo, accumulator: Accumulator) throws -> ResultOrPromise<Accumulator.PartialResult> {
    ...
    case .list(let innerType):
      guard let array = value as? [JSONValueAny] else { return .result(.failure(JSONDecodingError.wrongType)) }
